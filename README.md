...
HATCH_metadata.xlsx
        ...
        └─ 01.Kmeans_code.ipynb  →  Cluster_result.xlsx
                ...
                └─ 03.ASD_index.ipynb + Control_data.xlsx
                        ...
                        │        └─ 06.Figure 1c.R  →  Figure 1c.pdf, Figure 1c.xlsx
...
- `Figure 1c.pdf` — Fig 1c (within-cluster symptom-profile distances with BH-adjusted q values in scientific notation)
- `Figure 1c.xlsx` — pairwise Wilcoxon results, including p and BH-adjusted q values in scientific-text columns with three decimal places
...
## 15.Kmeans_stability.ipynb

Assesses K-means clustering stability across 500 runs using the adjusted Rand index (ARI) and label agreement with the reference ASD severity clustering.

Output files:

- `kmeans_seed_stability_500.pdf` — combined stability plots
- `panel_a_ARI.pdf` — ARI distribution
- `panel_b_agreement.pdf` — label agreement distribution
...
- `metadata_diet_per_cal.xlsx` — metadata input for scripts 08 and 15, including Cohort, reference Cluster labels and symptom subscales for script 15
...
- **Core Python dependencies:** pandas, NumPy, SciPy, scikit-learn and Matplotlib
